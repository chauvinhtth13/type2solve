...
const CORE_ASSETS = [
  ...
  './styles/fonts.css',
  './assets/fonts/noto-sans-vietnamese-full.woff2',
  './assets/fonts/noto-serif-vietnamese-full.woff2',
  ...
  './scripts/games/adventure/combat.js',
  './scripts/games/typing/content.js',
  './scripts/games/typing/index.js',
  './scripts/games/sudoku/index.js',
  './scripts/games/duel/rules.js',
  './scripts/games/duel/index.js',
  './scripts/games/nim/rules.js',
...
async function fetchAndCache(cache, asset, requestCache) {
  const url = scopedUrl(asset);
  const response = await fetch(new Request(url, { cache: requestCache }));
  if (!response || !response.ok) throw new Error(`Không tải được offline asset: ${asset}`);
  await cache.put(url, response);
}

async function cacheAll(assets, requestCache, bestEffort = false) {
  const cache = await caches.open(CACHE_NAME);
  const jobs = assets.map((asset) => fetchAndCache(cache, asset, requestCache));
  if (bestEffort) await Promise.allSettled(jobs);
...
async function warmDeferredCache() {
  ...
  if (pending.length) await cacheAll(pending, 'default', true);
}
