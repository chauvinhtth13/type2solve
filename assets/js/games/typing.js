...
(function typingBattle(global) {
  ...
  const FALLBACK_BEST_KEY = 'dttd_typing_best_v1';
  const FALLBACK_SETTINGS_KEY = 'dttd_typing_settings_v1';
  ...
  function playSound(name) {
    ...
    } catch (_) { /* âm thanh không được làm gián đoạn ván chơi */ }
  }
  ...
  function clearLocalTimers() {
    pendingTimers.forEach(timer => global.clearTimeout(timer));
    pendingTimers.clear();
  }
  ...
  function loadProfileSettings() {
    let settings = null;
    try {
      if (typeof GameStorage !== 'undefined' && GameStorage && typeof GameStorage.load === 'function') {
        settings = GameStorage.load().settings;
      }
    } catch (_) { /* dùng bản dự phòng bên dưới */ }
    if (!settings) {
      try { settings = JSON.parse(localStorage.getItem(FALLBACK_SETTINGS_KEY) || 'null'); }
  ...
  function saveProfileSettings(settings) {
    let saved = false;
    try {
      if (typeof GameStorage !== 'undefined' && GameStorage && typeof GameStorage.updateSettings === 'function') {
        GameStorage.updateSettings(settings);
        saved = true;
      }
    } catch (_) { saved = false; }
    if (!saved) {
      try { localStorage.setItem(FALLBACK_SETTINGS_KEY, JSON.stringify(settings)); }
  ...
  function readBestRecord() {
    ...
    try {
      if (typeof GameStorage !== 'undefined' && GameStorage && typeof GameStorage.load === 'function') {
        record = GameStorage.load().records.typing;
      }
    } catch (_) { record = null; }
    if (!record) {
      try { record = JSON.parse(localStorage.getItem(FALLBACK_BEST_KEY) || 'null'); }
      catch (_) { record = null; }
    }
    record = record || {};
  ...
  function resetTypingCampaign() {
    ...
    let saved = false;
    try {
      if (typeof GameStorage !== 'undefined' && GameStorage
        && typeof GameStorage.updateRecords === 'function') {
    ...
    if (!saved) {
      try {
        localStorage.setItem(FALLBACK_BEST_KEY, JSON.stringify({ ...previous, campaignCleared: -1 }));
        saved = true;
      } catch (_) { /* localStorage có thể bị chặn */ }
    }

    if (!saved) {
      setCampaignStatus('⚠️ Không thể lưu thao tác reset. Hãy kiểm tra quyền lưu trữ của trình duyệt.', 'error');
      return false;
    }
  ...
  function saveResultRecord(result) {
    ...
    try {
      if (typeof GameStorage !== 'undefined' && GameStorage) {
        if (typeof GameStorage.updateRecords === 'function') {
          GameStorage.updateRecords({ typing: best });
          saved = true;
        }
        if (result.stars > 0 && typeof GameStorage.addStars === 'function') GameStorage.addStars(result.stars);
      }
    } catch (_) { saved = false; }
    if (!saved) {
      try { localStorage.setItem(FALLBACK_BEST_KEY, JSON.stringify(best)); }
      catch (_) { /* vẫn cho phép chơi khi lưu trữ bị chặn */ }
    }
    try {
      if (typeof refreshProfileSummary === 'function') refreshProfileSummary();
    } catch (_) { /* giao diện hồ sơ là phần phụ */ }
  ...
  function finishTypingRun(won) {
    ...
    const stars = won
      ? Math.min(8, 3 + Math.floor(accuracy / 25) + (state.config.score >= 1.6 ? 1 : 0))
